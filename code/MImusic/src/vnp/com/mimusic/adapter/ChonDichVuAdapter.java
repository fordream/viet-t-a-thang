...
//		ImageLoaderUtils.getInstance(context).displayImage(service_icon, home_item_img_icon, R.drawable.no_image);
		...
		values.put("name", cursor.getString(cursor.getColumnIndex(DichVuStore.service_name)));
		values.put(DichVuStore.service_code, cursor.getString(cursor.getColumnIndex(DichVuStore.service_code)));
		values.put("content", cursor.getString(cursor.getColumnIndex(DichVuStore.service_content)));
